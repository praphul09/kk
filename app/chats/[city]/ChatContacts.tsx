        ...
        if (response.status === 200) {
            ...
            await Promise.all(data.data.map(async contact => {
                const response = await fetch('/api/sendMessageBulk', {
                    method: 'POST',
                    body: JSON.stringify({ to: contact['PhoneNumber'],  template,  "https://storage.googleapis.com/schoolinfo-bucket/images/"  + contact['Code'] + ".jpg", city: contactState?.city, dispname: contact['Title']}),
                    headers: headers
                })
                if (response.status === 200) {
                    setName('')
